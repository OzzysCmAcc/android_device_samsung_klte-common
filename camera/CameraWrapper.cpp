...
static char *camera_fixup_getparams(int id __unused, const char *settings)
{
    ...
    if (previewFormat) {
        if (!strcmp(previewFormat, "nv12-venus")) {
    ...
    /* Advertise video snapshot support */
    params.set(CameraParameters::KEY_VIDEO_SNAPSHOT_SUPPORTED, "true");

    /* Advertize zsl support in hdr mode */
    params.set("zsl-hdr-supported", "true");

    /* Properly advertise supported ISO values */
    params.set("iso-values",
...
static char *camera_fixup_setparams(int id, const char *settings)
{
    ...
    /* Send Samsung's hdr parameter on rear camera */
    const char *sceneMode = params.get(CameraParameters::KEY_SCENE_MODE);
    if (sceneMode) {
        if (id == REAR_CAMERA_ID && !strcmp(sceneMode, "hdr")) {
            params.set("rt-hdr", "on");
        } else {
            params.set("rt-hdr", "off");
        }
    }
    ...
    if (id == REAR_CAMERA_ID) {
        ...
        params.set("phase-af", "off");
        params.set("dynamic-range-control", "off");
    }
...
static int camera_start_recording(struct camera_device *device)
{
    ...
    if (is_4k_video(parameters)) {
        parameters.setPreviewFormat("nv12-venus");
